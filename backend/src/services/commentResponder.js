...
export class CommentResponder {
  ...
  async checkAndRespond() {
    ...
    try {
      ...
      this.logger.info(
        `✅ Comment check complete: ${totalProcessed} processed, ${totalResponded} responses`,
      );
  ...
  async processPostComments(post) {
    ...
    try {
      ...
      const commentsData = await this.api.getComments(post.id, {
        sort: "new",
        limit: 10,
      });
      ...
      this.logger.info(
        `Post ${post.id}: ${newComments.length} new comments to process`,
      );
      ...
      for (const comment of newComments) {
        ...
        if (!shouldRespond.respond) {
          this.logger.info(
            `Skipping comment ${comment.id}: ${shouldRespond.reason}`,
          );
          this.stats.commentsSkipped++;
          await this.markCommentProcessed(
            post.id,
            comment.id,
            "skipped",
        ...
        try {
          ...
          if (response) {
            ...
            this.logger.info(
              `✅ Responded to comment ${comment.id} by ${comment.agentName}`,
            );
          }
          ...
          this.logger.error(
            `Failed to respond to comment ${comment.id}:`,
            error.message,
          );
      ...
      this.logger.error(
        `Failed to process comments for post ${post.id}:`,
        error.message,
      );
  ...
  shouldRespondTo(comment) {
    ...
    const isRelevant =
      /analytics|insight|data|track|monitor|dashboard|solana|agent/i.test(
        comment.body,
      );
...
Response:`;
  ...
  async postResponse(postId, originalComment, responseText) {
    ...
    this.logger.info(
      `📝 Generated reasoning for comment response (confidence: ${(reasoning.confidence * 100).toFixed(1)}%)`,
    );
    ...
    if (this.solana && this.solana.canWrite()) {
      // ⬅️ ДОДАТИ ПЕРЕВІРКУ
      try {
        ...
        this.logger.info(
          `📝 Response logged on-chain: ${solanaTx.signature.slice(0, 16)}...`,
        );
        this.logger.info(`   Explorer: ${solanaTx.explorerUrl}`);
      } catch (solanaError) {
        this.logger.warn(
          "Failed to log response on-chain:",
          solanaError.message,
        );
      }
    }
