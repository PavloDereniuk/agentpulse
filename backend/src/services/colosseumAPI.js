  ...
  async getProjects(params = {}, useAPI = false, includeDrafts = true) {
    ...
    `,
      ...
      const projects = dbResult.rows.map((p) => ({
        ...
        repoLink: p.data?.repoLink || p.data?.github,
        demo: p.data?.demo,
        demoUrl: p.data?.demo,
        technicalDemoLink: p.data?.technicalDemoLink || p.data?.demo,
        liveAppLink: p.data?.liveAppLink,
        presentationLink: p.data?.presentationLink || p.data?.video,
        video: p.data?.video,
